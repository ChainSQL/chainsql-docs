...
drops_per_byte
---------------------
    该选项表示表交易中每字节数据消耗的drops，默认为1000，表示1字节数据消耗1000drops，范围为[1,10^6]
...
.. important:: 
    ...
    为了保证与低版本的API的兼容性，drops_per_byte的默认值为(10^6 /1024),表示1KB数据消耗1 ZXC。drops_per_byte不配置或者配置为默认值，保持与老版本API的向下兼容性；如果该值配置为非默认值，那么与老版本的API都不兼容，无法发起表交易。
