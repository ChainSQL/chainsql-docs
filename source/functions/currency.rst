...
数字资产发行的功能涉及到 ChainSQL 网络中的网关的概念以及网关发行数字资产，具体介绍如下：
...
在ChainSQL中，网关指的是数字资产在ChainSQL网络中流通经过的关口。网关的主要作用是发行数字资产，它允许使用者把法定数字资产注入、抽离ChainSQL网络，并可充当借、贷双方的桥梁；任意一个ZXC账户，都可以成为一个网关。
...
- 金融机构通过在ChainSQL网络中发行数字资产，可以加速资金流转，降低金融成本，最典型的例子是降低通过SWIFT跨国转账收取的高昂手续费；
- 金融机构可以利用区块链可靠，可追溯的特性，实现资金的自动清算，资金通过ChainSQL网络进行流通，不会出现对账不成功的问题，减少审计成本；
- 商家发行数字资产，可以作为一种积分、优惠券，可以很方便的实现积分的互转，并且所有记录自动上链，不可篡改；

术语
===============

- ``Trustline`` ，也就是信任通道，在ChainSQL中，一个账户只是把自己设置为网关是没有意义的，必须有人来信任它发行的数字资产才行，这个信任关系，就叫trustline。
- ``Rippling`` ，一个网关必须开启Rippling功能，信任它的账户之间才可以互转这一网关发行的数字资产，不然这一网关就是关闭的，发行的数字资产无法通过它进行流通。
- 费率( ``TransferRate`` )，数字资产的流通（转账或者挂单）会经过网关，网关可以选择设置费率对这些交易收取手续费，在ChainSQL中支持设置百分比作为费率。
- 发行账户( ``Issuing Address`` )，网关账户，也就是数字资产发行账户。
- 持币账户( ``Operatinal Address`` )，网关发行数字资产的过程，一般会涉及到另一个账户，也就是持币账户，网关发行的数字资产量会在最开始打到这个账户，后面由这个账户向其它账户进行转账。
...
下面举一个例子来说明，在ChainSQL网络中是如何通过网关来发行数字资产的，数字资产名称为AAA，要发行总量是10000。
...
2. 网关账户B：发行数字资产的账户，后续所有普通账户需要做信任操作，信任该账户发行的数字资产。

3. 普通账户C：普通账户,由发行数字资产的账户转账一定数量的数字资产给该账户，该账户用于数字资产的接收以及转账。
...
账户C和账户D信任网关账户B的数字资产AAA，信任的数字资产限额即数字资产发行数量10000，这个过程用到  ``TrustSet`` 交易
...
冻结发行数字资产主要是由网关发起，目的在于冻结已发行的数字资产。`详细信息 <https://xrpl.org/freezes.html#individual-freeze>`_


- RPC代码

账户 B 冻结发行给账户 D 的数字资产 ``AAA``, 额度为 10000
...
账户B 解冻发行给账户 D 的数字资产 ``AAA``, 额度为 10000
...
- `node.js数字资产发行 <https://github.com/ChainSQL/node-chainsql-api/blob/master/test/testRipple.js/>`_

- `JAVA 数字资产发行 <https://github.com/ChainSQL/java-chainsql-api/blob/master/chainsql/src/test/java/com/peersafe/example/chainsql/TestRipple.java/>`_
...
上面是用网关来发行定量数字资产的过程，如果只是作为积分发行，机构只需要在想要分发数字资产时，用持币账户给它下面的用户转账数字资产就可以了，每个用户需要做的是在最开始信任下网关，然后就可以在网络中进行数字资产的交易。
